...
finalize_yum_repo ()
{
  ...
  echo "Generating yum cache for github_git-lfs-source..."
  yum -q makecache -y --disablerepo='*' --enablerepo='github_git-lfs-source'
}

finalize_zypper_repo ()
{
  zypper --gpg-auto-import-keys refresh github_git-lfs
  zypper --gpg-auto-import-keys refresh github_git-lfs-source
}
